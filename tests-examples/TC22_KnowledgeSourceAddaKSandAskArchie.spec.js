...
test('Archie Knowledge Source - Add Knowledge Source and Ask Achie using the KS', async ({ page }) => {
    ...
    /*
    await page.getByRole('dialog').filter({ hasText: 'Archie' }).locator('iframe[title="sdkEmbedded"]').contentFrame().getByText('Browse').click();
    await page.getByRole('dialog').filter({ hasText: 'Archie' }).locator('iframe[title="sdkEmbedded"]').contentFrame().getByText('Browse or drag files hereMax').setInputFiles('C:/Users/DarwinDaluz/Documents/Smokeball Files/Archie Centralised Documents/Chronologies/Animal Cruelty Case.pdf');
...
await page.waitForTimeout(5000);
    ...
    await page.locator('iframe[title="sdkEmbedded"]').contentFrame().locator('button[id^="radix-"]').first().click();
