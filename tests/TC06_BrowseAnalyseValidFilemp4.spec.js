...
test('Archie Browse Analyse or review a document - Valid File mp4', async ({ page }) => {
  test.setTimeout(200000);
    ...
    await expect(sdkFrame.getByRole('heading', { name: 'video_with_transcript.mp4' })).toBeVisible();
    await page.waitForTimeout(2000);

    await expect(sdkFrame.getByText('video_with_transcript.mp4RemoveFile Previewvideo_with_transcript.mp4Optimising')).toBeVisible();
