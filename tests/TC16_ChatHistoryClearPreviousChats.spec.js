...
test('Archie Chat History Clear Previous Chats', async ({ page }) => {
  test.setTimeout(200000);
    ...
    await page.locator('iframe[title="sdkEmbedded"]').contentFrame().locator('.inline-flex.items-center.justify-center.rounded-md').first().click();
    await page.locator('iframe[title="sdkEmbedded"]').contentFrame().getByRole('button', { name: 'Delete' }).click();
    
    await page.waitForTimeout(1000);

    await expect(page.locator('iframe[title="sdkEmbedded"]').contentFrame().getByRole('status')).toContainText('Chat deleted');
    await page.waitForTimeout(2000);
    console.log('Previous Chat Deleted');
