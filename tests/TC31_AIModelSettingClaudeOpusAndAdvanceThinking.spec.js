...
test('Archie AI Model Setting Claude Opus with and without Advance Thinking', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'Search' }).fill('Claude Opus');
    await page.locator('a').filter({ hasText: 'openClaude Opus - Opus,' }).click();
    ...
    await page.waitForTimeout(3000);
    await page.locator('iframe[title="sdkEmbedded"]').contentFrame().getByRole('option', { name: 'Claude Opus' }).click();
    await page.locator('iframe[title="sdkEmbedded"]').contentFrame().getByRole('button', { name: 'Save' }).click();
    await page.waitForTimeout(3000);
    console.log('Archie AI Model Claude Opus Selected');

    await expect(page.locator('iframe[title="sdkEmbedded"]').contentFrame().getByLabel('Chat & Draft')).toContainText('Claude Opus 4');
    ...
    console.log('Ask Archie using AI Model Claude Opus');
    ...
    await page.locator('iframe[title="sdkEmbedded"]').contentFrame().getByRole('button', { name: 'Save' }).click();
    console.log('Archie AI Model Claude Opus with Advance Thinking Selected');
    await page.waitForTimeout(3000);
    ...
    await page.locator('iframe[title="sdkEmbedded"]').contentFrame().locator('.inline-flex.items-center.justify-center.rounded-md.text-sm.font-medium.ring-offset-background.transition-colors.hover\\:cursor-pointer.focus-visible\\:outline-none.focus-visible\\:ring-2.focus-visible\\:ring-ring.focus-visible\\:ring-offset-2.disabled\\:pointer-events-none.disabled\\:opacity-50.size-8').click();
    console.log('Ask Archie using AI Model Claude Opus with Advance Thinking');
    ...
    console.log('Archie AI Model Claude Opus Test Completed');
