...
test('Archie Browse Analyse or review a document - Invalid File JPG', async ({ page }) => {
  test.setTimeout(200000);
    ...
    await expect(sdkFrame.getByText('Archie cannot answer')).toBeVisible();
    await expect(sdkFrame.getByRole('dialog')).toContainText('Archie cannot answer questions on .jpg files. Please choose another file to proceed.');
    await page.waitForTimeout(5000);
    await sdkFrame.getByRole('button', { name: 'OK' }).click();
